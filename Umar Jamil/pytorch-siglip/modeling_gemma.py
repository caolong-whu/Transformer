...
class GemmaForCausalLM(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.lm_head = nn.Linear(config.hidde_size, config.vocab_size, bias=False)
...
class PaliGemmaModalProjector(nn.Linear):
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(
        ...
        text_mask = (inputs_ids != self.config.image_token_index) & (inputs_ids != self.config.pad_token_id)
        # [batch_szie, seq_len] True for image tokens
        image_mask = inputs_ids == self.config.image_token_index
        # [batch_size, seq_len] True for padding tokens
        pad_mask = inputs_ids == self.config.pad_token_id
